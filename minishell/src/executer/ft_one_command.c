...
static int ft_handle_redirections(t_redirect *redir, int *saved_stdin, int *saved_stdout)
{
    if (!redir)
        return (0);
...
static int	ft_prepare_heredocs(t_shell *shell, t_parser *parsed)
{
	...
	while (redir)
	{
		if (redir->type == R_HERE && !redir->document)
			redir->document = ft_get_heredoc_input(redir->file_name, shell, *parsed);
		redir = redir->next;
	}
	return (0);
}
...
int	ft_one_command(t_shell *shell, t_parser *parsed)
{
	...
	saved_stdin = -1;
	saved_stdout = -1;
	if (ft_handle_redirections(parsed->redirect, &saved_stdin, &saved_stdout) == -1)
		return (1);
