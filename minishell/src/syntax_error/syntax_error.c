...
/*   Updated: 2025/08/07 23:17:17 by saincesu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	r_append_error(t_token *token, t_shell *shell)
{
	if (token->type == R_APPEND)
	{
		...
		if (is_operator(token->next->content[0]))
		{
			ft_putstr_fd("minishell: syntax error near unexpected token"
				"`>>'\n", 2);
			shell->exit_code = 2;
...
static int	r_here_error(t_token *token, t_shell *shell)
{
	if (!token->next)
	{
		ft_putstr_fd("minishell: syntax error near unexpected token"
...
static int	r_in_and_r_out_error(t_token *token, t_shell *shell)
{
	if (!token->next)
	{
		ft_putstr_fd("minishell: syntax error near unexpected token"
...
static int	pipe_error(char *str, t_token *token, t_shell *shell)
{
	if (!token->next)
	{
